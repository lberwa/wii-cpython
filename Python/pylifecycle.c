...
#include "pycore_crossinterp.h"   // _PyXI_Init(), _PyXI_InitTypes()
...
Py_Init_Custom(const char** import_paths, size_t *count)
{
    ...
    (void)PyRun_SimpleString(
        ...
        "                pass\n"
#endif
        "        _slash = self.path.rfind('/')\n"
        "        _pdir = self.path[:_slash] if _slash >= 0 else 'sd:'\n"
        "        _base = self.path[_slash+1:] if _slash >= 0 else self.path\n"
        "        _pyc = (_pdir + '/__pycache__/'\n"
        "            + (_base[:-3] if _base.endswith('.py') else _base) + '.pyc')\n"
        "        _pyc_code = None\n"
        "        try:\n"
        "            import marshal as _m, struct as _s\n"
        "            _st = os.stat(self.path)\n"
        "            _sm, _ss = int(_st.st_mtime), _st.st_size\n"
        "            with open(_pyc, 'rb') as _f:\n"
        "                _raw = _f.read()\n"
        "            if len(_raw) >= 9 and _raw[:1] == b'\\x00':\n"
        "                _pm, _ps = _s.unpack_from('<II', _raw, 1)\n"
        "                if _pm == _sm and _ps == _ss:\n"
        "                    _pyc_code = _m.loads(_raw[9:])\n"
        "        except Exception:\n"
        "            pass\n"
        "        if _pyc_code is not None:\n"
        "            exec(_pyc_code, module.__dict__)\n"
        "            return\n"
        "        try:\n"
        "            code = compile(data, self.path, 'exec')\n"
        "        except SyntaxError as e:\n"
        "            #print('[WiiSourceLoader] SyntaxError in', self.path, ':', e)\n"
        "            raise\n"
        "        try:\n"
        "            import marshal as _m, struct as _s\n"
        "            _st = os.stat(self.path)\n"
        "            try:\n"
        "                os.mkdir(_pdir + '/__pycache__')\n"
        "            except OSError:\n"
        "                pass\n"
        "            _raw = (b'\\x00'\n"
        "                + _s.pack('<II', int(_st.st_mtime), _st.st_size)\n"
        "                + _m.dumps(code))\n"
        ...
        "                    _el = importlib.machinery.ExtensionFileLoader(\n"
        "                        fullname, _so)\n"
        "                    return _wii_spec(fullname, _el, _so)\n"
        "            except Exception:\n"
        "                pass\n"
        "            _mod = _base + _name + '.py'\n"
        "            try:\n"
        "                if os.path.isfile(_mod):\n"
        "                    return _wii_spec(fullname,\n"
        "                        WiiSourceLoader(fullname, _mod), _mod)\n"
        "            except Exception:\n"
        "                pass\n"
        "            _pkg_dir = _base + _name\n"
        "            _init = _pkg_dir + '/__init__.py'\n"
        "            try:\n"
        "                if os.path.isfile(_init):\n"
        "                    _ldr = WiiSourceLoader(fullname, _init,\n"
        "                        is_package=True, pkg_path=_pkg_dir)\n"
        "                    _spec = importlib.machinery.ModuleSpec(\n"
        "                        fullname, _ldr, origin=_init)\n"
        ...
        "                    _spec = importlib.machinery.ModuleSpec(fullname, None)\n"
        "                    _spec.submodule_search_locations = [_pkg_dir]\n"
        "                    return _spec\n"
        "            except Exception:\n"
        "                pass\n"
