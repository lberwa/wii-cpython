...
{
#ifdef DEBUG2
  curl_wii_tprint("curl: Curl_conn_cf_connect()");
#endif
  if(cf)
  {
    CURLcode result = cf->cft->do_connect(cf, data, done);
#ifdef DEBUG2
    curl_wii_tprint_i("curl: Curl_conn_cf_connect rc=", (int)result);
    if(done) {
      char line[96];
      snprintf(line, sizeof(line), "curl: Curl_conn_cf_connect done=%d", (int)(*done));
      terminal_print(line);
    }
#endif
    return result;
...
#ifdef DEBUG2
  curl_wii_tprint("curl: Curl_conn_connect()");
#endif
...
#ifdef DEBUG2
    curl_wii_tprint_i("curl: Curl_conn_connect rc=", (int)result);
    if(done) {
      char line[96];
      snprintf(line, sizeof(line), "curl: Curl_conn_connect done=%d", (int)(*done));
      terminal_print(line);
    }
#endif
