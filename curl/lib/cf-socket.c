...
static void curl_wii_tprint(const char *msg)
{
  if(msg)
    terminal_print(msg);
}
static void curl_wii_tprint_i(const char *prefix, int v)
{
  char buf[96];
...
#else
static void curl_wii_tprint(const char *msg) { (void)msg; }
static void curl_wii_tprint_i(const char *prefix, int v)
{
  (void)prefix;
  (void)v;
                            ...
                            curl_socket_t *sockfd)
{
  ...
  {
    char line[128];
    terminal_print("---------- socket_open: start ----------");
    snprintf(line, sizeof(line), "socket_open: family=%d socktype=%d proto=%d",
             (int)addr->family, (int)addr->socktype, (int)addr->protocol);
    terminal_print(line);
  }
#endif
  ...
  if(data->set.fopensocket) {
    ...
    *sockfd = CURL_SOCKET(addr->family, addr->socktype, addr->protocol);
    ...
    {
      char line[96];
      snprintf(line, sizeof(line), "socket_open: sockfd=%d errno=%d",
               (int)*sockfd, (int)SOCKERRNO);
      terminal_print(line);
    }
#endif
  ...
  if(*sockfd == CURL_SOCKET_BAD) {
    ...
    {
      char line[96];
      snprintf(line, sizeof(line), "socket_open: failed errno=%d",
               (int)SOCKERRNO);
      terminal_print(line);
      terminal_print("---------- socket_open: fail after socket() ----------");
    }
#endif
  ...
  if(fcntl(*sockfd, F_SETFD, FD_CLOEXEC) < 0) {
    ...
    {
      char line[96];
      snprintf(line, sizeof(line), "socket_open: fcntl(F_SETFD) errno=%d",
               (int)SOCKERRNO);
      terminal_print(line);
      terminal_print("---------- socket_open: fail after fcntl ----------");
    }
#endif
                               ...
                               struct Curl_easy *data)
{
...
#ifdef DEBUG2
  curl_wii_tprint("curl: cf_socket_open()");
#endif
  DEBUGASSERT(ctx->sock == CURL_SOCKET_BAD);
  ctx->started_at = *Curl_pgrs_now(data);
#ifdef DEBUG2
  curl_wii_tprint("---------- cf_socket_open: start ----------");
#endif
#ifdef SOCK_NONBLOCK
...
#ifdef DEBUG2
  curl_wii_tprint_i("cf_socket_open: socket_open rc=", (int)result);
#endif
#ifdef SOCK_NONBLOCK
  ...
  if(result) {
#ifdef DEBUG2
    curl_wii_tprint("---------- cf_socket_open: fail after socket_open ----------");
#endif
    goto out;
  }
...
#ifdef DEBUG2
  curl_wii_tprint_i("cf_socket_open: set_remote_ip rc=", (int)result);
#endif
  if(result) {
#ifdef DEBUG2
    curl_wii_tprint("---------- cf_socket_open: fail after set_remote_ip ----------");
#endif
    goto out;
  }
...
#ifdef DEBUG2
      curl_wii_tprint_i("cf_socket_open: fsockopt error=", error);
      curl_wii_tprint("---------- cf_socket_open: fail after fsockopt ----------");
#endif
      goto out;
    }
  }
...
#ifdef DEBUG2
    curl_wii_tprint_i("cf_socket_open: bindlocal rc=", (int)result);
#endif
    if(result) {
...
#ifdef DEBUG2
  curl_wii_tprint_i("cf_socket_open: curlx_nonblock rc=", error);
#endif
  if(error < 0) {
    result = CURLE_UNSUPPORTED_PROTOCOL;
    ctx->error = SOCKERRNO;
#ifdef DEBUG2
    curl_wii_tprint("---------- cf_socket_open: fail after curlx_nonblock ----------");
#endif
    goto out;
  }
#else
...
out:
  ...
  {
    char line[96];
    snprintf(line, sizeof(line), "curl: cf_socket_open result=%d", (int)result);
    terminal_print(line);
    curl_wii_tprint("---------- cf_socket_open: end ----------");
  }
#endif
                      ...
                      bool is_tcp_fastopen)
{
  ...
  (void)data;
#ifdef DEBUG2
  curl_wii_tprint("curl: do_connect()");
#endif
  if(is_tcp_fastopen) {
...
#ifdef DEBUG2
  curl_wii_tprint_i("curl: connect rc=", rc);
#endif
  return rc;
}
                               ...
                               bool *done)
{
...
#ifdef CURL_WII
  curl_wii_tprint("curl: cf_tcp_connect()");
#endif
  if(cf->connected) {
    *done = TRUE;
    return CURLE_OK;
  ...
  *done = FALSE; /* a negative world view is best */
  if(ctx->sock == CURL_SOCKET_BAD) {
    ...
    set_local_ip(cf, data);
#ifdef DEBUG2
    curl_wii_tprint_i("curl: connect errno=", error);
#endif
    CURL_TRC_CF(data, cf, "local address %s port %d...",
                ctx->ip.local_ip, ctx->ip.local_port);
#ifdef CURL_WII
    /* Wii/libogc net_connect is blocking and does not support
       non-blocking connect verification via getsockopt(SO_ERROR). */
    if(rc == 0) {
      ctx->connected_at = *Curl_pgrs_now(data);
      *done = TRUE;
      cf->connected = TRUE;
      CURL_TRC_CF(data, cf, "connected (devkitPPC) on fd=%" FMT_SOCKET_T,
                  ctx->sock);
      return CURLE_OK;
    }
#endif
...
#ifdef DEBUG2
  curl_wii_tprint_i("curl: SOCKET_WRITABLE rc=", rc);
#endif
  ...
  if(rc == 0) { /* no connection yet */
    ...
    if(verifyconnect(ctx->sock, &ctx->error)) {
      ...
      *done = TRUE;
      cf->connected = TRUE;
#ifdef DEBUG2
      curl_wii_tprint("curl: verifyconnect ok");
#endif
